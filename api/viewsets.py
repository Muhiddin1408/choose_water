...
class Product_FilialViewset(viewsets.ModelViewSet):
    ...
    @action(methods=['post'], detail=False)
    def bloc_to_piece(self, request):
        try:
            product1 = request.data['product1']
            product2 = request.data['product2']
            bloc = request.data['bloc']
            piece = request.data['piece']

            product_1 = ReceiveItem.objects.filter(product_id=product1).last()
            print(product_1.quantity)
            product_1.quantity = product_1.quantity - bloc
            product_1.save()
            print(product_1.quantity)
            product_2 = ReceiveItem.objects.filter(product_id=product2).last()
            print(product_2.quantity)
            product_2.quantity = product_2.quantity + piece
            product_2.save()
            print(product_2.quantity)
            return Response("Amaliyot yaxshi yakunlandi!!", status.HTTP_200_OK)

        except:
            return Response("Amaliyot yakunlanmadi!!", status.HTTP_404_NOT_FOUND)
...
class ReceiveItemViewset(viewsets.ModelViewSet):
    ...
    serializer_class = ReceiveItemGetSerializer
    filter_backends = (DjangoFilterBackend,)
    ...
    def create_list(self, request):
        ...
        print('data', data)
        for d in data:
            serializer = ReceiveItemSerializer(data=d)
            # serializer = self.get_serializer(data=request.data)
    ...
    def by_faktura(self, request):
        ...
        for i in data:
            print()
            serializer = ReceiveItemSerializer(data=i)
            print('serializer')
            serializer.is_valid(raise_exception=True)
            valid_data = serializer.validated_data
            print('vali')
            ...
            print(faktura)
            product = ProductFilial.objects.filter(name=name, barcode=barcode, expired_date=expired_date, ombor=True).last()
...
class HisobdanChiqarishOmborViewset(viewsets.ModelViewSet):
    ...
    def by_id(self, request):
        ...
        serializer = ProductFilialSerializer(b)
        return Response(serializer.data)
